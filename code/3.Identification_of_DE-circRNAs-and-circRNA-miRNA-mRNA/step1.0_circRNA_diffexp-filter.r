...
#lme function
runlme.My.NoAdjust2 = function(circID,datRPM,datTraits,label){
  oneRPM <- datRPM %>% 
...
LME_result <- LME_result %>% dplyr::mutate(LME_diff_NoAdjust = purrr::map(.x=SampleList,function(.x){
		...
		#.x=list(PD1_postNonBenefitID,PD1_postBenefitID);
		print(times)		
		times=times+1
		...
		for (i in 1:numCirc){ 
			circID <- circV[i+1]
			result <- try(runlme.My.NoAdjust2(circID,temp_datRPM,temp_datTraits,label), silent=T)
			if(length(result) == 6){
